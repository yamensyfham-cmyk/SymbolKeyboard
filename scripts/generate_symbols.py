...
def read_cached_file(path: str) -> str:
    if not os.path.exists(path):
        print(f"Cached file not found at {path}")
        return None
    print(f"Reading cached UnicodeData.txt from {path}...")
    with open(path, "r", encoding="utf-8-sig") as f:
        data = f.read()
    print(f"Read {len(data)} bytes from cache")
    return data
...
def determine_block_name(code_point: int) -> str:
    blocks = [
        ...
        (0x11E00, 0x11E5F, "Makasar"),
        (0x11EC0, 0x11EFF, "Lisu Supplement"),
        (0x11F00, 0x11F5F, "Tamil Supplement"),
        (0x11FB0, 0x11FBF, "CJK Unified Ideographs Extension G"),
...
def create_cached_fallback(output_path: str):
    ...
    if not os.path.exists(DEFAULT_CACHED):
        print("Creating placeholder cached/UnicodeData.txt")
        with open(DEFAULT_CACHED, "w", encoding="utf-8") as f:
            f.write("")
...
def main():
    ...
    if not os.path.exists(args.cached):
        print("Warning: cached UnicodeData.txt not found.")

    data = download_unicode_data(UNICODE_DATA_URL)
    if data is None:
        print("Download failed, falling back to cached file...")
